...
- **Worktree Manager (M1).** Creates, lists, and removes git worktrees by shelling out to `git worktree`. Used by the new-session flow: given a base branch and a task name, it creates a worktree at a project-configured location and returns its path, which the Attachment Driver then uses to spawn `claude` inside a new tmux window. Conventions settled in M1 design:
