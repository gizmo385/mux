...
A fast, terminal-first multiplexer for managing multiple terminal-agent conversations across local and remote hosts. Claude Code is the reference agent — the default, and the only one enabled without configuration; OpenAI Codex and Pi are supported as opt-in additional agents (see the Agent CLI glossary entry and Roadmap).

The user keeps several agent sessions running in parallel — some on the local workstation, some on remote SSH-reachable machines. agent-mux is the dashboard that tracks them all in one TUI: which projects they cover, which are awaiting input, what they're currently doing. Pressing Enter on a session brings the active session up inside an embedded terminal pane next to the dashboard list; the user sees their existing tmux + agent session and can type, paste, slash-command, anything the agent CLI supports. The dashboard sidebar stays visible while the user works in the embedded pane — see other sessions' attention state in real time without detaching. The tool is a control plane on top of `tmux` and the agent's on-disk transcripts; it does not re-implement either.

The target user is a single developer who already lives in the terminal, already uses tmux, and runs coding-agent conversations (Claude Code, and optionally Codex or Pi) frequently enough that "which window was that conversation in again?" is a real friction point.
...
- **Project** — the working directory the agent CLI runs in for a session. For sessions agent-mux created (M1+) this is usually a worktree of a known Repo — except for the `N` no-worktree flow (post-M5), where agent-mux spawns directly in the repo root. For sessions started externally, it can be any directory.
...
- **Session discovery.** On startup, scans each enabled agent's transcript directory (Claude Code's `~/.claude/projects` by default) for existing sessions and populates the dashboard. The user does not have to register sessions manually.
...
- **Spawn a new session (M1).** From the dashboard, the user picks a repo from the registry, names a task, and picks a base branch; agent-mux creates a git worktree, starts the agent CLI (`claude` by default, or the agent chosen in the picker step when ≥2 are enabled) inside it, and registers the resulting session. Post-M5, the new session lands in the embedded pane next to the sidebar (same path as a normal attach) rather than a fullscreen handoff. The user is in the new session immediately. Worktree-backed sessions group under their parent repo's header in the dashboard (resolved from each cwd's `.git` pointer file at discovery time), so a session in `discord` and one in a `discord-<task>` worktree share one project group instead of fragmenting into per-worktree entries. The new-session picker pre-selects the repo (or at least the host) of the currently-highlighted session so spawning a sibling stays in-context.
- **Spawn a new session without a worktree (post-M5).** A sibling flow bound to `N` (Shift+n): the user picks a repo, and agent-mux spawns the agent CLI (`claude` by default) directly in the repo root — no `git worktree add`, no task name, no base branch, no `.agent-mux/task.toml`. For quick exploratory chats or attaching to a checkout the user already has live edits in. Same picker UI as `n` (mode marked in the modal title); same embedded-pane delivery.
- **Attach.** Pressing Enter on a session opens the active session inside an embedded terminal pane occupying the right side of the dashboard; the dashboard list collapses to a compact ~40-column sidebar on the left. The sidebar widens (to ~60 columns, clamped so the session pane keeps a usable width) when focus returns to it via `Ctrl-a Esc`, so longer session titles are legible while the user is navigating, and snaps back to compact when focus returns to the session. Inside the pane the user sees their real tmux + agent session and can type, paste, use slash commands, anything the agent CLI supports — agent-mux is just hosting the terminal tmux runs in. The sidebar updates live while the embedded pane has focus, so the user can see attention transitions on other sessions without detaching. `Ctrl-a Esc` returns focus to the sidebar without killing the PTY; pressing Enter on a different row drops the old PTY and attaches to the new session. `--no-embed` opts back into the legacy `tmux switch-client` / `SuspendAndRun` flow for users who prefer it.
- **Spawn terminal in session cwd.** A keybind opens a new tmux window in the session's working directory, so the user can run shell commands alongside the agent without leaving agent-mux's mental model.
- **Open a file the agent edited.** The user can jump straight into a file the session's agent has changed, in their own editor, without hunting for it. A `[[tools]]` binding whose command references `{file}` (e.g. `vim {file}`) is *file-scoped*: pressing its key surfaces the list of files that session edited (derived from the transcript — the agent's edit operations, most-recent-first; for Claude Code, `Edit`/`Write`/`MultiEdit`/`NotebookEdit`) and opens the picked one via the same tool-launch path a plain tool uses. Works for local and remote sessions alike (the editor opens against the path on the session's host).
- **Remote sessions (M2).** Sessions on configured SSH hosts appear in the same dashboard. Attaching opens an `ssh -t target tmux attach …` inside the embedded pane (or as a foreground subprocess with `--no-embed`) — same dispatch as local. Attention state for remote sessions is detected the same way as local: by watching the remote transcript file over the existing SSH connection. Post-M5: `n` against a remote-host repo creates the worktree on the remote and spawns the agent CLI there, surfacing the new session in the dashboard via the normal discovery pipeline.
...
- **Multiple agent CLIs (post-M5 arc, shipped 2026-07-09/10).** Sessions from other qualifying agent CLIs (Codex, Pi) appear in the same dashboard once enabled via `[agents.<name>]` config: discovered from each agent's transcript tree, attention-tracked through each agent's transcript semantics, attachable and spawnable through the same flows. When ≥2 agents are enabled the dashboard marks each session row (and quickswitcher entry) with a dim agent tag, and the `n`/`N` new-session flow gains an agent-selection step; with a single enabled agent both are suppressed, so with no `[agents]` config the behaviour and appearance are identical to Claude-Code-only. Per-agent capability gaps (e.g. Codex approval prompts, which never persist to its transcript) are documented in the README rather than papered over; Codex's blocked-on-approval state is covered by its lifecycle-hook mechanism (`agent-mux install-hooks --agent codex`, feeding the same marker pipeline as the Claude Code Notification hook). The Codex and Pi *read* paths were verified live 2026-07-10 against real `codex` 0.142.5 and `pi` 0.80.6 (discovery, title, attention render correctly for all three agents together; a real Codex `task_*` event-rename was caught and fixed there); spawn/adoption and the hook path remain synthetic-fixture-only until a live authed run (current-release best-effort; see README).
...
- **Multi-agent CLI support (decided 2026-07-09, shipped 2026-07-09/10).** The `AgentCli` trait extraction plus Codex and Pi as the first non-Claude agents, executed as WP0–WP9 per `docs/plans/2026-07-09-multi-agent-cli.md`. Live end-to-end verification against real `codex`/`pi` binaries is the standing follow-up.
