...
- surface how to return to the dashboard from an attached session (from user, 2026-05-16, dogfooding M1). Today there is no in-app affordance; the answer depends on launch mode and the user has to know tmux. **Inside-tmux:** `attach` runs `tmux switch-client`, so there is no parent/child relationship to break — the user must use `prefix + s/w/l` to navigate back to the agent-mux window manually. **Outside-tmux:** `attach` suspends agent-mux and runs `tmux attach` as a subprocess, so `prefix + d` detaches the subprocess and the TUI resumes automatically. Smallest move: extend the dashboard footer with a mode-aware "return: prefix+d / prefix+s" hint. Larger move: in inside-tmux mode, name the agent-mux window predictably (e.g. `agent-mux`) so the hint can say "prefix+: switch-client -t agent-mux". `#m1 #ui #attachment`
