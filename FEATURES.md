...
- ✓ remote session discovery: each `[hosts.<name>]` in config gets a background thread at startup that opens its `ControlMaster`, runs discovery against the host's `transcript_root`, pre-computes initial attention against the same connection, and posts results back to the dashboard. Local sessions appear immediately; remote sessions stream in as their hosts connect (parallel, not blocking). Connect failures surface in the status bar (`connect <name>: …`). Footer shows "connecting to N host(s)…" while pending. **Limitation:** remote sessions' attention is frozen at the startup reading — live polling lands in the next M2 chunk (`watcher` extension); attaching into a remote tmux lands in the chunk after that.
