...
- **Notifier (M4).** Owns the per-session suppression state that decides whether to fire an OS notification for a given attention transition. Called from the catalog's `update_attention` boundary in `main.rs` — that's the only place a true transition signal exists, because `update_attention` returns the previous attention value alongside writing the new one. The notifier itself stays decoupled from the catalog (it accepts a `Transition` struct of `&str`/`&Path` labels, not a `Session`) so swapping in a different attention source later requires no notifier changes. The actual OS call lives behind a `Dispatcher` trait; production uses `LibNotifyDispatcher` (which delegates to `notify-rust` and spawns a one-shot thread per dispatch so D-Bus/NSUserNotification latency never reaches the UI thread), tests use a recorder. Suppression is two-layered: an episodic flag (set on dispatch, cleared when the session leaves `NeedsInput`) collapses repeated transitions within the same "assistant just stopped" event, and a short time-window debounce on top of that absorbs Working↔NeedsInput flapping at the watcher's poll cadence. Failed dispatch doesn't arm the suppression — a libnotify outage stays self-healing rather than silently muting the user. User-facing config (on/off, sound, quiet hours, per-host suppression) is deferred to M5's broader config surface.
