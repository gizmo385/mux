...
- **`CARGO_REGISTRY_TOKEN`** — a [crates.io API token](https://crates.io/me) scoped to publish updates of `agent-mux`. release-plz's `release` command invokes `cargo publish` against this token when a merged Release PR ships a new version. Note: release-plz refuses to compute a next version when a git tag exists for a name that *doesn't* exist on the registry — the "did we forget to publish?" check fires before the publish step runs. If the project enabled crates.io after the first GitHub tag was already cut (which agent-mux did), do a one-time manual `cargo login && cargo publish` from a clean `main` to seed the registry with whatever version `Cargo.toml` currently has. Subsequent versions go through release-plz normally.
