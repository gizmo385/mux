...
M0 shipped (skeleton + discovery + transcript watcher + tmux attach with the outside-tmux and resume-from-transcript fallbacks). M1 in progress: WorktreeManager and the AttachmentDriver.spawn_session extension have landed; the workspace-folder Config, Repo Registry, and new-session UI flow are next.
