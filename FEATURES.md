...
- ✓ `SshHost`: shells out to the system `ssh` binary, reusing one `ControlMaster` connection per host so each operation is a local-socket round-trip rather than a full TCP+TLS+auth handshake. Drop guard runs `ssh -O exit`; ControlPersist=600 caps stranded masters at 10 min of idle on the remote side if the guard never fires. Remote `find -printf '%T@ %p\0'` for transcript listing, `cat` / `tail -c` for reads, `test -d` for is-dir. Not yet wired into discovery or the watcher (separate chunks).
